...
각 회전들을 배열에 적용한 뒤, 그 회전에 의해 위치가 바뀐 숫자들 중 가장 작은 숫자들을 순서대로 
...
def solution(rows, columns, queries):
    ...
    map_list=[[k for k in range(1+i*columns,(i+1)*columns+1)] for i in range(0, rows)]
    ...
    for x1, y1, x2, y2 in queries:
        ...
        for y in range(y1,y2):
            tmp1=map_list[x1][y+1]
            map_list[x1][y+1]=tmp
            tmp=tmp1
        #아래쪽으로
        for x in range(x1,x2):
            tmp1=map_list[x+1][y2]
            map_list[x+1][y2]=tmp
            tmp=tmp1
        #왼쪽으로
        for y in range(y2,y1,-1):
            tmp1=map_list[x2][y-1]
            map_list[x2][y-1]=tmp
            tmp=tmp1
        #위쪽으로
        for x in range(x2,x1,-1):
            tmp1=map_list[x-1][y1]
            map_list[x-1][y1]=tmp
            tmp=tmp1
            
        tmp=[]
        tmp.extend(map_list[x1][y1:y2+1])
        tmp.extend(map_list[x2][y1:y2+1])
        tmp.extend([map_list[x][y1] for x in range(x1,x2+1)])
        tmp.extend([map_list[x][y2] for x in range(x1,x2+1)])
