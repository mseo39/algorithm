...
철수의 토마토 농장에서 토마토를 보관하는 큰 창고를 가지고 있다. 토마토는 아래의 그림과 같이 격자 상자의 탄에 하나씩 넣어서 창고에 보관한다
...
def dfs():
    ...
    while queue:
        ...
        for i in distance:
            b_x=n_x+i[0]
            b_y=n_y+i[1]
            if 0<=b_x<N and 0<=b_y<M and map_list[b_x][b_y]==0:
                queue.append([b_x, b_y, n_d+1])
                map_list[b_x][b_y]=1
    return n_d if chk() else -1
